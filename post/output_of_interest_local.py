...
for fracs_path in paths:
    ...
    grad_s = fracs["m_sal_onshore"].differentiate("time") * -1
    grad_fw_on = fracs["fw_onshore"].differentiate("time") * -1 
    
    #%%Outputs of interest
    oi = {}
    oi["end_fw"]          = fracs["fw"].isel(time=-1).values
    oi["onshore_fw"]      = fracs["fw_onshore"].isel(time=-1).values
    oi["offshore_fw"]     = fracs["fw_offshore"].isel(time=-1).values
    oi["onshore_bw"]      = fracs["bw_onshore"].isel(time=-1).values
    ...
    oi["s_gradient"]      = (grad_s).isel( 
                                time=slice(-3, None)
                                ).mean().values
    oi["fw_on_gradient"]  = (grad_fw_on).isel( 
                                time=slice(-3, None)
                                ).mean().values
