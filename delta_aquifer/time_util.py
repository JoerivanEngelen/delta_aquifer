...
def subdivide_time(t, max_perlen):
    ...
    sub_ts = split_list(t, list(splits)+[len(perlens)])
    first_el = np.array([sub[0] for sub in sub_ts])
    starts, ends = first_el[:-1], first_el[1:]
    
    sub_ends = ends-starts
    sub_t = [sub-start for sub, start in zip(sub_ts, starts)]    
    return(sub_t, sub_ends)